...
//        include.plugins('jquery'); //always load jQuery
        include.plugins('jquery-1.4.4'); //always load jQuery
		...
		if(include.options.env == 'production' && include.options.loadProduction){
			// XXX Wingu modification
			// allow the revision number to be tacked onto the script location
			// so we can prevent caching between releases
			var _script = include.options.production;
			if (include.options.revision) {
				_script += '?v=' + include.options.revision;
			}
            document.write('<script type="text/javascript" src="'+_script+'"></script>' );
            return
        }
